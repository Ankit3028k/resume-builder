...
const Resume = ({ resumeData }) => {
  ...
  const downloadPDF = () => {
    ...
    doc.setFont("helvetica", "normal");
    doc.text(
      `Email: ${resumeData.email || "N/A"} | Mobile: ${
        resumeData.mobile || "N/A"
      }`,
      10,
      y
    );
    y += 5;
    doc.text(
      `LinkedIn: ${resumeData.linkedin || "N/A"} | Portfolio: ${
        resumeData.portfolio || "N/A"
      }`,
      10,
      y
    );
    ...
    doc.setFontSize(10);
    doc.setFont("helvetica", "normal");
    y = addTextWithLineBreak(
      doc,
      `Frontend: ${resumeData.technicalskill?.frontend || "N/A"}`,
      y
    );
    y = addTextWithLineBreak(
      doc,
      `Backend: ${resumeData.technicalskill?.backend || "N/A"}`,
      y
    );
    y = addTextWithLineBreak(
      doc,
      `Tools: ${resumeData.technicalskill?.tools || "N/A"}`,
      y
    );
    y += 6;
    ...
    resumeData.ExtracurricularInvolvement?.forEach((Extra) => {
      ...
      y = addTextWithLineBreak(
        doc,
        `${Extra.role} (${Extra.organization}) (${Extra.dates})`,
        y
      );
